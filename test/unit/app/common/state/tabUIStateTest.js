...
describe('tabUIState unit tests', function () {
  ...
  describe('checkIfTextColorBlackOrWhite', function () {
    it('returns an empty string if frame is null/undefined', function * () {
      assert.equal(tabUIState.checkIfTextColorBlackOrWhite(), false)
    })
    ...
    it('returns true if colors match', function * () {
      ...
      const result = tabUIState.checkIfTextColorBlackOrWhite(state, frameKey, 'black')
  ...
  describe('getTabEndIconBackgroundColor', function () {
    it('returns an empty string if frame is null/undefined', function * () {
      assert.equal(tabUIState.getTabEndIconBackgroundColor(), false)
    })

    it('defaults to `theme.tab.background`', function * () {
      const state = defaultState.set('activeFrameKey', 1337)
      const result = tabUIState.getTabEndIconBackgroundColor(state, frameKey)
      assert.equal(result, theme.tab.background)
    })

    it('returns themeColor if tab is active', function * () {
      const state = defaultState
    ...
    it('returns `transparent` if tab is active but no themeColor', function * () {
      const state = defaultState
        .setIn(['frames', index, 'themeColor'], false)
      const result = tabUIState.getTabEndIconBackgroundColor(state, frameKey)
      assert.equal(result, 'transparent')
    })

    it('returns `transparent` if tab is partitioned', function * () {
      const state = defaultState
        .setIn(['frames', index, 'partitionNumber'], 1337)
      const result = tabUIState.getTabEndIconBackgroundColor(state, frameKey)
      assert.equal(result, 'transparent')
    ...
    it('returns `transparent` if tab is private', function * () {
      const state = defaultState
        .setIn(['frames', index, 'isPrivate'], true)
      const result = tabUIState.getTabEndIconBackgroundColor(state, frameKey)
      assert.equal(result, 'transparent')
    ...
    it('returns `transparent` if tab is intersected', function * () {
      const state = defaultState
        .setIn(['ui', 'tabs', 'intersectionRatio'], intersection.at35)
      const result = tabUIState.getTabEndIconBackgroundColor(state, frameKey)
      assert.equal(result, 'transparent')
